...
class Test(TestCase):
    ...
    def test_precedence_classes_with_dimension_and_attribute_values(self):
        ...
        with self.assertRaises(ClassesPrecedencePropertyError) as error:
            _ = self.precedence.precedence(["qb:DimensionProperty", "qb:AttributeProperty"])
    ...
    def test_precedence_classes_with_attribute_and_measure_values(self):
        with self.assertRaises(ClassesPrecedencePropertyError) as error:
            _ = self.precedence.precedence(["qb:AttributeProperty", "qb:MeasureProperty"])
    ...
    def test_precedence_classes_with_dimension_and_measure_values(self):
        with self.assertRaises(ClassesPrecedencePropertyError) as error:
            _ = self.precedence.precedence(["qb:DimensionProperty", "qb:MeasureProperty"])
    ...
    def test_precedence_classes_with_class_values(self):
        with self.assertRaises(ClassesPrecedenceClassError) as error:
            _ = self.precedence.precedence(["rdfs:Class", "owl:Class"])

        self.assertEqual(str(error.exception), "['rdfs:Class', 'owl:Class'] -> Possible redundant Class definition")

# print(precedence(['qb:AttributeProperty, qb:CodedProperty']))
# print(precedence(['qb:CodedProperty, qb:DimensionProperty']))
# print(precedence(['qb:DimensionProperty, qb:CodedProperty']))
# print(precedence(['skos:Concept, <http://bauhaus/codes/concept/AjustementSaisonnier>']))
# print(precedence(['<http://bauhaus/codes/concept/HebergementNombresEtoiles>, skos:Concept']))
